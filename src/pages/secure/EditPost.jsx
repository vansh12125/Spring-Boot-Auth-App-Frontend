...
import { ArrowLeft, Send, AlertCircle, CircleCheck, Globe, Lock, Loader2 } from "lucide-react";
...
export default function EditPost() {
  ...
  const fetchPostDetails = async () => {
    try {
      setIsLoading(true);
      const response = await getPostByIdAndValidateUser(postId,user?.userId);
      ...
      setIsPublic(response.data.public !== undefined ? response.data.public : true);
      setIsLoading(false);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updatePost(postId,user?.userId, updatedData);
