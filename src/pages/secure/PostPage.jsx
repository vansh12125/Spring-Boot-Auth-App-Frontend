...
import { ArrowLeft, Clock, Heart, MessageSquare, AlertCircle, Loader2 } from "lucide-react";
...
export default function PostPage() {
  ...
  const fetchPostDetails = async () => {
    try {
      ...
      const normalizedData = {
        ...response.data,
        id: response.data.postId || response.data.id || response.data._id,
        likes: Array.isArray(response.data.likes) ? response.data.likes : [],
        hasLiked: Array.isArray(response.data.likes) && response.data.likes.includes(currentUserId)
      };
      setPost(normalizedData);
    } catch (err) {
      setError(err?.response?.data?.message || "This post could not be found. It might have been deleted.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLike = async () => {
    ...
    const previouslyLiked = post.likes.includes(currentUserId);
                ...
                >
                  <Heart
                    className={`w-4 h-4 transition-all duration-200 group-active:scale-90 ${
                      post.hasLiked ? "fill-current text-red-500" : "text-gray-500"
                    }`}
                  />
